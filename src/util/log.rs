use {
    crate::{prelude::*, strings::STRING_DATA},
    ::log::{Level, LevelFilter},
    bevy::prelude::{App, Plugin, StartupStage},
    serde::Deserialize,
    std::{
        collections::{hash_map::Iter as StdIter, HashMap},
        fmt,
        iter::Peekable,
        sync::Once,
    },
...
fn usize_to_level(val: usize) -> Level {
    unsafe {
        std::mem::transmute(crate::clamp!(
            val,
            Level::Error as usize,
...
impl LogError {
    pub fn log(&self) -> () {
        ...
        if !self.error.is_empty() {
            error! (target: self.target.as_str(), "{}", self.error);
            has_logged = true;
        }
        if !self.warn.is_empty() {
            warn!  (target: self.target.as_str(), "{}", self.warn );
            has_logged = true;
        }
        if !self.info.is_empty() {
            info!  (target: self.target.as_str(), "{}", self.info );
            has_logged = true;
        }
        if !self.debug.is_empty() {
            debug! (target: self.target.as_str(), "{}", self.debug);
            has_logged = true;
        }
        if !self.trace.is_empty() {
            trace! (target: self.target.as_str(), "{}", self.trace);
            has_logged = true;
        }
        if !has_logged {
            eprintln!(
                "log() called on empty LogError with target \"{}\"",
                self.target
            );
        }
    }
