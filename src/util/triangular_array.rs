...
pub const fn triangle_number(n: usize) -> usize { n * (n + 1_usize) / 2_usize }
...
impl<const SL: usize> Helpers<SL> {
	...
	pub const TRI_COUNT: usize = 2_usize * triangle_number(Self::SIDE_LEN_MINUS_1) - Self::SIDE_LEN_MINUS_1;

	pub const fn other_index(index_1: usize, index_2: usize) -> usize { Self::INDEX_SUM - index_1 - index_2 }
}
...
#[macro_export]
macro_rules! Helpers { ($t:ty) => { <$t as UseHelpers>::Helpers }; }
...
impl<T: Clone + Default, const SL: usize, const FS: usize> TriangularArray<T, SL, FS> {
	...
	pub const fn side_len() -> usize { <Helpers!(Self)>::SIDE_LEN }

	pub const fn side_len_minus_1() -> usize { <Helpers!(Self)>::SIDE_LEN_MINUS_1 }
...
impl<T: Clone + Default, const SL: usize, const FS: usize> AsMut<TriangularArray<T, SL, FS>> for [T; FS] {
...
> IndexMut<(&'a IndexMap<SL>, &'a Index<SL>)> for TriangularArray<T, SL, FS> {
	fn index_mut(&mut self, (index_map, index): (&'a IndexMap<SL>, &'a Index<SL>)) -> &mut Self::Output {
...
impl<
	...
	const SL: usize,
	const FS: usize
> IndexMut<usize> for TriangularArray<T, SL, FS> {
...
impl<
	...
	const SL: usize,
	const FS: usize
> IndexTrait<usize> for TriangularArray<T, SL, FS> {
	type Output = T;

	fn index(&self, index: usize) -> &Self::Output { &self.0[index] }
}

impl<T: Clone + Default, const SL: usize, const FS: usize> UseHelpers for TriangularArray<T, SL, FS> {
...
impl IndexType2D {
	...
	fn next(self) -> Self { unsafe { transmute::<u8, Self>((self as u8 + 1_u8) % Self::COUNT) } }

	fn prev(self) -> Self { unsafe { transmute::<u8, Self>((self as u8 + Self::COUNT - 1_u8) % Self::COUNT) } }
}
...
impl<const SL: usize> Index<SL> {
	pub fn corner(index_type: IndexType2D) -> Self { Self {
		index_type,
	...
	pub fn new(index_type: IndexType2D, index_1: usize, index_2: usize) -> Self { Self {
		index_type,
		index_1,
		index_2,
		_pd: Default::default()
	...
	pub fn iter_row_indices(self) -> RowIndicesIterator<SL> { RowIndicesIterator::<SL> {
		index: self,
	...
	pub fn iter_all_indices() -> AllIndicesIterator<SL> { AllIndicesIterator::<SL>::default() }
	...
	pub fn iter_border_indices() -> BorderIndicesIterator<SL> { BorderIndicesIterator::<SL>::default() }
	...
	#[inline(always)]
	pub fn index_1_is_border(&self) -> bool { self.index_1 == <Helpers!(Self)>::SIDE_LEN_MINUS_1 }

	#[inline(always)]
	pub fn index_2_is_border(&self) -> bool { self.index_2 == <Helpers!(Self)>::SIDE_LEN_MINUS_1 }

	#[inline(always)]
	pub fn other_index_is_border(&self) -> bool { self.index_1 + self.index_2 == <Helpers!(Self)>::SIDE_LEN_MINUS_1 }

	pub fn is_border(&self) -> bool {
		self.index_1_is_border() || self.index_2_is_border() || self.other_index_is_border()
	}

	pub fn is_first_corner(&self) -> bool { self.index_1_is_border() && self.other_index_is_border() }

	pub fn is_last_corner(&self) -> bool { self.index_1_is_border() && self.index_2_is_border() }

	pub fn next_type(&self) -> Self { Self {
	...
	pub fn prev_type(&self) -> Self { Self {
	...
	pub fn first_in_row(&self) -> Self { Self {
	...
	pub fn last_in_row(&self) -> Self { Self {
		index_type:	self.index_type,
		index_1:	self.index_1,
	...
	pub fn prev_in_row(mut self) -> Option<Self> {
		if (<Helpers!(Self)>::SIDE_LEN - self.index_1 .. <Helpers!(Self)>::SIDE_LEN).contains(&self.index_2) {
...
impl<const SL: usize> Default for Index<SL> { fn default() -> Self { Self::corner(IndexType2D::IJ) } }
...
impl<'im, I: Iterator<Item = Index<SL>>, const SL: usize> Iterator for MappedIndexIterator<'im, I, SL> {
	type Item = usize;

	fn next(&mut self) -> Option<usize> { self.iter.next().map(|index: Index<SL>| -> usize { self.index_map[&index] }) }
}

impl<'im, I: Iterator<Item = Index<SL>> + DoubleEndedIterator, const SL: usize>
	DoubleEndedIterator for MappedIndexIterator<'im, I, SL>
{
	fn next_back(&mut self) -> Option<usize> {
		self.iter.next_back().map(|index: Index<SL>| -> usize { self.index_map[&index] })
	}
}
...
impl<const SL: usize> Iterator for RowIndicesIterator<SL> {
	...
	fn next(&mut self) -> Option<Index<SL>> {
		if self.index.index_2 < self.end {
			...
			self.index = self.index.next_in_row().unwrap_or_else(|| -> Index<SL> { Index::<SL> {
				index_2: self.end,
				.. self.index
			} });
...
pub type MappedRowIndicesIterator<'im, const SL: usize> = MappedIndexIterator<'im, RowIndicesIterator<SL>, SL>;
...
pub type MappedAllIndicesIterator<'im, const SL: usize> = MappedIndexIterator<'im, AllIndicesIterator<SL>, SL>;
...
pub type MappedBorderIndicesIterator<'im, const SL: usize> = MappedIndexIterator<'im, BorderIndicesIterator<SL>, SL>;
...
impl<'im, const SL: usize> MappedTrioIndicesIterator<'im, SL> {
	pub fn new(index_map: &'im IndexMap<SL>) -> Self { Self {
		index_map,
		...
		// Initialize jk and ki 2D arrays
		{
			...
			for jk in 0_usize .. <Helpers!(Self)>::SIDE_LEN {
				...
				for ki in 0_usize .. <Helpers!(Self)>::SIDE_LEN {
					...
					/* ij could be invalid here, pointing to cells outside the actual triangle. Check for that, and use
					usize::MAX in that case*/
					if ij < <Helpers!(Self)>::SIDE_LEN {
						/* In both the cases where we iterate on j then k or k then i, we already have one of the
						variables used in the ij 2D array. Variable ij represents the missing index of the two for the
						corresponding cell. Fill these two tables then (jk and ki) using the already established indices
...
mod tests {
	...
	fn test_iter_trios() -> () {
		assert_eq!(
			TestIndex::iter_mapped_trio_indices(&*INDEX_MAP).collect::<Vec<(usize, usize, usize)>>(),
			&[
				(0, 1, 2),
				(1, 3, 4),
				(2, 4, 5),
