use bevy_inspector_egui::{
    egui::{Button, CollapsingHeader, Color32, RichText, Ui},
    Context, Inspectable,
};
...
impl<'a, T: Default + KVP> InspectableBinMapMut<'a, T> {
    fn label_button(ui: &mut Ui, text: &str, text_color: Color32) -> bool {
        ui.add(Button::new(RichText::new(text).color(text_color)).frame(false))
            .clicked()
    }
    ...
    pub fn get(&mut self, k: &<T as KVP>::Key) -> Option<&mut <T as KVP>::Value> {
        match self
            .map
            .binary_search_by_key(k, |t: &T| -> <T as KVP>::Key { t.key().clone() })
        {
...
impl<'a, T: Default + KVP> Inspectable for InspectableBinMapMut<'a, T> {
    type Attributes = (
        <<T as KVP>::Key as Inspectable>::Attributes,
        <<T as KVP>::Value as Inspectable>::Attributes,
    );
    ...
    fn ui(&mut self, ui: &mut Ui, options: Self::Attributes, context: &mut Context) -> bool {
        ...
        ui.vertical(|ui: &mut Ui| {
            ...
            if key_changed {
                self.map
                    .sort_by_cached_key(|t: &T| -> <T as KVP>::Key { t.key().clone() });
                self.map
                    .dedup_by_key(|t: &mut T| -> <T as KVP>::Key { t.key().clone() });
            }
        });
