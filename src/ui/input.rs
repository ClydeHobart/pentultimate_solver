use {
    ...
    crate::{
        app::{prelude::*, SaveState},
        math::polyhedra::{
            data::{Data, FaceData},
            Polyhedron,
        },
        piece::consts::*,
        preferences::{AnimationSpeedData, RandomizationType, SpeedData},
        prelude::*,
        puzzle::{
            transformation::{
                Action, Addr, FullAddr, FullMask, GenusIndex, GenusIndexType, HalfAddr, Library,
                RandHalfAddrParams,
            },
            ExtendedPuzzleState, InflatedPieceStateComponent as PSC, InflatedPuzzleState,
        },
    },
    bevy::{
        app::CoreStage,
    ...
    bevy_inspector_egui::{egui, Context, Inspectable},
    bit_field::BitField,
    egui::{CollapsingHeader, Color32, ComboBox, Context as EguiContext, Grid, Ui},
...
impl IntoKeyCode for u32 {
    fn into_key_code(self) -> KeyCode {
        unsafe { transmute::<Self, KeyCode>(self) }
    }
}
...
impl Debug for Modifier {
    fn fmt(&self, f: &mut Formatter) -> FmtResult {
        match self {
            Self::Alt => f.write_str("Opt"),
            Self::Ctrl => f.write_str("Ctrl"),
            Self::Shift => f.write_str("Shift"),
            Self::Win => f.write_str("Cmd"),
        }
    }
}
...
impl From<usize> for Modifier {
    fn from(modifier: usize) -> Self {
        unsafe { transmute::<u8, Self>(modifier as u8) }
    }
}
...
impl From<KeyPress> for String {
    fn from(key_press: KeyPress) -> Self {
        ...
        write!(&mut string, "{:?}", unsafe {
            transmute::<u32, KeyCode>((key_press.0 & KeyPress::KEY_CODE_MASK) as u32)
        })
        .unwrap();
                            ...
                            .show(ui, |ui: &mut Ui| {
                                for modifier in 0_usize..Modifier::COUNT {
                                                      ...
                                                      key_code: &dyn Fn(Modifier) -> KeyCode|
                                     {
                                        ui.centered_and_justified(|ui: &mut Ui| {
                                            ...
                                            ui.visuals_mut().override_text_color =
                                                Some(if key_press.0 & mask != 0_u16 {
                                            ...
                                            if ui
                                                .button(format!("{:?}", key_code(modifier)))
                                                .clicked()
                                            {
                                                key_press.0 ^= mask;
                                                changed = true;
                                    ...
                                    button(
                                        ...
                                        ui,
                                        &Modifier::left_mask,
                                        &Modifier::left_key_code,
                                    );
                                    button(
                                        self,
                                        ui,
                                        &Modifier::right_mask,
                                        &Modifier::right_key_code,
                                    );
                                    ui.end_row();
                                }
                            });
...
impl TryFrom<&str> for KeyPress {
    ...
    fn try_from(str_slice: &str) -> Result<Self, String> {
        ...
        for c in str_slice.chars() {
            if let Ok(modifier) = Modifier::try_from(c) {
                if !left_right.left && !left_right.right {
                    return Err(format!(
                        "Modifier {:?} has both left and right keys disabled",
                        modifier
                    ));
                }
        ...
        if let Some(key_code_str) = str_slice.get(curr_byte..str_slice.len()) {
            ...
            Err(format!(
                "couldn't grab slice starting at byte {}",
                curr_byte
            ))
        }
    }
        ...
        match self.try_get_key_code_index(&key_code) {
            Ok(key_code_index) => {
                self.key_code_to_actions[key_code_index]
                    .actions
                    .set_bit(action_index, true);
            }
            Err(key_code_index) => self.key_code_to_actions.insert(
                key_code_index,
                KeyCodeWithActions {
                    key_code,
...
impl<'a> From<&'a Rc<*mut KeyPresses>> for &'a mut KeyPresses {
    fn from(rc_self: &Rc<*mut KeyPresses>) -> Self {
        unsafe { (*rc_self).as_mut() }.unwrap()
    }
}
...
impl Index<KeyPressAction> for KeyPresses {
    ...
    fn index(&self, action: KeyPressAction) -> &KeyPress {
        &self.action_to_key_press[action as usize]
    }
}
            ...
            for (action_index, key_press) in action_to_key_press.iter_mut().enumerate() {
                ...
                let action_changed: bool = key_press.ui(
                    ...
                    Some(Rc::new(move |key_press: KeyPress| -> Result<(), String> {
                        ...
                        if let Ok(key_code_index) = key_presses.try_get_key_code_index(&key_code) {
                            let mut actions: ActionsBitField =
                                key_presses.key_code_to_actions[key_code_index].actions;
                            let mut clashing_action_index: usize =
                                actions.trailing_zeros() as usize;

                            while clashing_action_index < KEY_PRESS_ACTION_COUNT {
                                let clashing_key_press: KeyPress =
                                    key_presses.action_to_key_press[clashing_action_index];
                                if clashing_action_index != action_index
                                    && clashing_key_press.clashes(key_press)
                                {
                                    return Err(format!(
                                        "Clashes with {:?}: {:?}",
                ...
                if action_changed {
                    ...
                    <&mut KeyPresses>::from(&rc_self)
                        .update_key_press_for_action(action_index, *key_press);
                }
...
impl CurrentAction {
    pub fn s_now(&self) -> Option<f32> {
        if self.duration.is_zero() {
            ...
            let s: f32 =
                (Instant::now() - self.start).as_millis() as f32 / self.duration.as_millis() as f32;
...
impl PendingActions {
    fn pop_current_action(
        ...
        if let (Some(action), Some(camera_orientation)) = (
            self.actions.pop_front(),
            CameraQueryMutNT(camera_query).orientation(
                |camera_orientation: Option<&mut Quat>| -> Option<Quat> {
                    camera_orientation.copied()
                },
            ),
        ) {
            Some(CurrentAction {
    ...
    pub fn randomize(
        ...
        puzzle_state: &InflatedPuzzleState,
        ...
        let (puzzle_state, actions): (InflatedPuzzleState, VecDeque<Action>) = {
            ...
            let genus_indices: Vec<GenusIndex> = {
                ...
                for genus_index_usize in 0_usize..Library::get_genus_count() {
                    if preferences
                        .file_menu
                        .randomization_params
                        .random_transformation_genera
                        .get_bit(genus_index_usize)
                    {
                        genus_indices.push(
                            GenusIndex::try_from(genus_index_usize as GenusIndexType).unwrap(),
                        );
                    }
                }
            ...
            let mut actions: VecDeque<Action> =
                VecDeque::<Action>::with_capacity(random_transformation_count);
            ...
            while actions.len() < random_transformation_count {
                let transformation: FullAddr = FullAddr::from((
                    ...
                    HalfAddr::from(RandHalfAddrParams {
                        thread_rng: &mut thread_rng,
                        allow_species_large_indices: false,
                    }),
                ));
                ...
                if let Some(prev_action) = actions.back() {
                    ...
                    if match (
                        ...
                        (GenusIndex::REORIENTATION, GenusIndex::REORIENTATION) => true,
                        (GenusIndex::SIMPLE, GenusIndex::SIMPLE) => {
                            transformation.get_species_index()
                                == prev_transformation.get_species_index()
                        }
    ...
    pub fn load(save_state: &SaveState, animation_speed_data: &AnimationSpeedData) -> Self {
        Self {
        ...
        loop {
            if self.current_action.is_none() {
                    ...
                    .map(Box::<PendingActions>::as_mut)
                {
                    ...
                    self.current_action =
                        pending_actions.pop_current_action(&mut queries.p0(), self.action_type);
                }
            }
            ...
            if let Some(current_action) = &self.current_action {
                ...
                match current_action.s_now() {
                        ...
                        if action.transformation.is_valid() {
                            ...
                            for comprising_simple in comprising_simples {
                                ...
                                if cycles == 0.0_f32 {
                                    ...
                                    for mut piece_components_mut_item in queries.p1().iter_mut() {
                                        let piece_index: usize =
                                            piece_components_mut_item.piece_component.index;
                    ...
                    None => {
                        ...
                        if action.transformation.is_valid() {
                            let standardization_addr: FullAddr =
                                action.standardization().as_reorientation();
                        ...
                        match self.action_type {
                            ...
                            | PuzzleActionType::Solve => {
                                if warn_expect!(action.transformation.is_valid()) {
                                    extended_puzzle_state
                                        .actions
                                        .truncate(extended_puzzle_state.curr_action);
...
impl FileAction {
    ...
    fn poll(&mut self) -> Option<Option<FileHandle>> {
        if self.check_ready() {
            ...
            if let Poll::Ready(file_handle) =
                self.future.get_mut().unwrap().as_mut().poll(&mut context)
            {
                Some(file_handle)
            } else {
                None
    ...
    fn check_ready(&mut self) -> bool {
        self.ready.0.swap(false, Ordering::Relaxed)
    }
}
...
impl InputToggles {
    ...
    fn update(&mut self, key_presses: &KeyPresses, keyboard_input: &Input<KeyCode>) {
        ...
        if let Some(should_increment) =
            if key_presses[KPA::CycleFamilyDown].is_active(keyboard_input) {
                Some(true)
        ...
        } else if let Some(should_increment) =
            if key_presses[KPA::CycleGenusUpWithinFamily].is_active(keyboard_input) {
                Some(true)
            ...
            }
        {
            ...
            self.genus_index = GenusIndex::try_from(
                (genus_index - base_genus_index
                    + if should_increment {
                        1 as GIT
                    } else {
                        family_genus_count - 1 as GIT
                    })
                    % family_genus_count
                    + base_genus_index,
            )
                ...
                let (mouse_motion_delta, mouse_wheel_delta): (Vec2, f32) = {
                    ...
                    (
                        if mouse_button_input.pressed(MouseButton::Middle) {
                            mouse_motion_events
                                .iter()
                                .map(|mouse_motion: &MouseMotion| -> &Vec2 { &mouse_motion.delta })
                                .sum::<Vec2>()
                                / time_delta
                        } else {
                            Vec2::ZERO
                        ...
                        mouse_wheel_events
                            .iter()
                            .map(|mouse_wheel: &MouseWheel| -> f32 { mouse_wheel.y })
                            .sum::<f32>()
                            / time_delta,
                    )
                };
                ...
                if !mouse_motion_delta.abs_diff_eq(Vec2::ZERO, f32::EPSILON) {
                    (
                        Quat::from_axis_angle(
                            Vec3::new(mouse_motion_delta.x, -mouse_motion_delta.y, 0.0_f32)
                                .cross(Vec3::Z)
                                .normalize(),
                            mouse_motion_delta.length() / PAN_SCALING_FACTOR
                                * preferences.speed.camera.pan_speed as f32,
                        ),
                        true,
                    )
                    ...
                    (
                        Quat::from_rotation_z(
                            mouse_wheel_delta / ROLL_SCALING_FACTOR
                                * preferences.speed.camera.roll_speed as f32,
                        ),
                        true,
                    )
                } else {
            ...
            (None, Some(file_action)) => {
                if let Some(file_handle) = file_action.poll() {
                    if let Some(Some(file_name)) =
                        file_handle.as_ref().map(FileHandle::path).map(Path::to_str)
                    {
                        match file_action.action_type {
                            FileActionType::Save => {
                                warn_expect_ok!(SaveState {
                                    ...
                                    camera: CameraQueryNT(&camera_query).orientation(
            ...
            (Some(active_action), None) => {
                if input_state.has_camera_rotation {
                    if matches!(active_action.action_type, PuzzleActionType::RecenterCamera) {
                        /* If the whole purpose of the current active transformation action is to
                        recenter the camera, which we no longer wish to do, end the active
                        transformation action entirely */
    ...
    fn update_action(
        ...
        if let Some(puzzle_action_type) = puzzle_action_type {
            let camera_orientation: Quat = CameraQueryNT(camera_query)
                .orientation(|camera_orientation: Option<&Quat>| -> Option<Quat> {
                    camera_orientation.copied()
                })
                .unwrap_or_default();
            let camera_start: HalfAddr = CameraPlugin::compute_camera_addr(&camera_orientation);
            let recenter_camera: bool =
                matches!(puzzle_action_type, PuzzleActionType::RecenterCamera);
            let action: Action = match puzzle_action_type {
                PuzzleActionType::RecenterCamera => Action::new(FullAddr::default(), camera_start),
                PuzzleActionType::Transformation => Action::new(
                    FullAddr::from((
                        input_state.toggles.genus_index,
                        input_state.toggles.half_addr(default_species.unwrap()),
                    )) + camera_start,
                    camera_start,
                ),
                PuzzleActionType::Undo => extended_puzzle_state.actions
                    [extended_puzzle_state.curr_action - 1_usize]
                    .invert(),
                PuzzleActionType::Redo => {
                    extended_puzzle_state.actions[extended_puzzle_state.curr_action]
                }
                _ => unreachable!(),
            };
            ...
            let (camera_orientation, has_camera_orientation): (Quat, bool) =
                if recenter_camera || input_state.toggles.enable_recentering {
            ...
            let duration: Duration =
                action.compute_duration(&speed_data.animation, puzzle_action_type);
...
impl Plugin for InputPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(InputState::default())
            .add_system_to_stage(
                CoreStage::PreUpdate,
                Self::run.after(EguiSystem::BeginFrame),
            );
    }
}
