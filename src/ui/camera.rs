...
impl Update for LightAndCameraData {
    fn update(&self, other: &Self, world: &mut World, _: &Preferences) -> () {
        if self != other {
            ...
            if let Some((mut transform, mut bevy_perspective_projection)) = world
...
impl<'field, 'world, 'state, 'c> CameraQueryNT<'field, 'world, 'state, 'c> {
    pub fn orientation<T, F: FnOnce(Option<&Quat>) -> T>(&self, f: F) -> T {
        match self.0.iter().next() {
            Some(camera_components_item) => f(Some(&camera_components_item.transform.rotation)),
            None => f(None),
        }
    }
}
...
pub struct CameraQueryMutNT<'field, 'world, 'state, 'c>(
    pub &'field mut CameraQueryMut<'world, 'state, 'c>,
);
...
impl<'field, 'c> CameraQueryMutStateNT<'field, 'c> {
    pub fn orientation<T, F: FnOnce(Option<&mut Quat>) -> T>(
        &mut self,
        world: &mut World,
        f: F,
    ) -> T {
        ...
        commands
            .spawn_bundle((
                ...
                Transform::from_rotation(
                    Data::get(Polyhedron::Icosidodecahedron).faces[Type::Pentagon.index_offset()]
                        .quat,
                ),
            ...
            .with_children(|child_builder: &mut ChildBuilder| -> () {
                ...
                child_builder.spawn_bundle(DirectionalLightBundle {
                    ...
                    transform: Transform::from_translation(Vec3::from(
                        light_and_camera_data.light_pos,
                    ))
                ...
                child_builder.spawn_bundle(PerspectiveCameraBundle {
                    ...
                    transform: Transform::from_translation(Vec3::from(
                        light_and_camera_data.camera_pos,
                    ))
                    .looking_at(Vec3::ZERO, Vec3::Y),
