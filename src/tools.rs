...
mod uses {
    pub use {
        crate::{
            ...
            prelude::*,
            puzzle::{
                transformation::{Action, HalfAddr, HalfAddrAttrs},
                InflatedPieceStateComponent, InflatedPuzzleState,
            },
        ...
        bitvec::{array::BitArray, BitArr},
        egui::{widgets::Separator, Button, CollapsingHeader, Color32, Ui, Vec2},
        num_traits::One,
...
mod data {
    pub use crate::puzzle::{
        solver::tools::Solver,
        tools::{PuzzleState, Stack},
    };

    use super::{uses::*, Tool};

    /* We need this trait to be able to restrict ToolData with it while still have ToolDataBox still
    compile, since Inspectable::setup() has no self argument */
    pub trait InvokeUi {
...
impl ToolsData {
    ...
    pub fn should_render(&self) -> bool {
        !self.tool_data_boxes.is_empty()
    }

    pub fn is_tool_active(&self, tool: Tool) -> bool {
        self.active_tools[tool as usize]
    }
    ...
    fn from(tool_iter: T) -> Self {
        ...
        for tool_index in 0_usize..Tool::COUNT {
            if tools_data.active_tools[tool_index] {
                tools_data
                    .tool_data_boxes
                    .push(Tool::try_from(tool_index).unwrap().new_data());
            }
        }
    ...
    fn ui(&mut self, ui: &mut Ui, _: (), _: &mut Context) -> bool {
        ...
        for tool_index in 0_usize..Tool::COUNT {
            ...
            if ui.checkbox(&mut tool_bool, tool.as_str()).changed() {
                ...
                if tool_bool {
                    self.tool_data_boxes
                        .insert(tool_data_index, tool.new_data());
                } else {
...
impl PartialEq for ToolsData {
    fn eq(&self, other: &Self) -> bool {
        if self.active_tools == other.active_tools {
            for (self_tool_data, other_tool_data) in self
                .tool_data_boxes
                .iter()
                .zip(other.tool_data_boxes.iter())
            {
