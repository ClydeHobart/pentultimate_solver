use {
    super::explorer::{Explorer, ExplorerParams, IsEndState, RunResult, ShouldExplore},
    crate::{
        app::prelude::*,
        math::polyhedra::{data::Data, Polyhedron},
        piece::consts::*,
        prelude::*,
        puzzle::{
            inflated::{PosAndRot, PuzzleState},
            transformation::{
                Action, FullAddr, FullMask, GenusIndexBitArray, GenusRange, HalfMask, Library,
            },
        },
    ...
    bevy::prelude::*,
    bitvec::prelude::*,
    egui::{Color32, Grid, RichText, Ui, Vec2, Visuals},
    std::{
...
const PENTAGON_HALF_MASK: HalfMask =
    ((1 as HalfMask) << usize::PENTAGON_PIECE_COUNT) - 1 as HalfMask;
...
impl SolverData {
    fn new() -> Self {
        ...
        let mut pent_dot_products: Vec<i8> =
            Vec::<i8>::with_capacity(COMPLETION_TIER_COUNT + 1_usize);
        let mut tri_dot_products: Vec<i8> =
            Vec::<i8>::with_capacity(COMPLETION_TIER_COUNT + 1_usize);
        ...
        for (piece_index, face_data) in icosidodecahedron_data.faces.iter().enumerate() {
            /* Multiply the dot product by -1 so that sorting the vectors (in ascending order) is in
            decreasing order of alignment with origin_norm  */
        ...
        for (piece_index, dot_product) in dot_products.iter().enumerate() {
            solver_data.focus_half_masks[(if piece_index < TRIANGLE_INDEX_OFFSET {
                &pent_dot_products
            } else {
                &tri_dot_products
            })
        ...
        for completion_half_mask_index in 0_usize..COMPLETION_TIER_COUNT {
            solver_data.completed_half_masks[completion_half_mask_index + 1_usize] |=
                solver_data.completed_half_masks[completion_half_mask_index];
        }

        for solver_genus_index in 0_usize..SolverGenusIndex::Count as usize {
            let solver_genus_index_string: String = format!("{:?}", unsafe {
                transmute::<usize, SolverGenusIndex>(solver_genus_index)
            });
            ...
            if let Ok(genus_range) = GenusRange::try_from(solver_genus_index_string.as_str()) {
                ...
                log::warn!(
                    "Couldn't find genus index for string \"{}\"",
                    solver_genus_index_string
                );
            }
        }
...
impl<'p, 'r> From<PosAndRot<'p, 'r>> for SolveStage {
    fn from(pos_and_rot: PosAndRot) -> Self {
        FullMask::from(pos_and_rot).into()
    }
}
...
impl From<FullMask> for Completion {
    fn from(full_mask: FullMask) -> Self {
        (full_mask, SolveStage::from(full_mask)).into()
    }
}
...
impl FullMaskAndCompletion {
    ...
    fn get_focus_affected_half_mask(&self) -> HalfMask {
        self.completion
            .solve_stage
            .get_focus_affected_half_mask(&self.full_mask)
    }
    ...
    fn get_focus_unaffected_half_mask(&self) -> HalfMask {
        self.completion
            .solve_stage
            .get_focus_unaffected_half_mask(&self.full_mask)
    }

    fn get_incorrect_piece_count(&self) -> u32 {
        (SOLVER_DATA.completed_half_masks[self.completion.tier as usize + 1_usize]
            & self.get_focus_affected_half_mask())
        .count_ones()
    }
    ...
    fn immut_ui(&self, ui: &mut Ui) {
        ui.collapsing("Full Mask And Completion", |ui: &mut Ui| {
            ...
            ui.scope(|ui: &mut Ui| {
                ...
                let completed_tier_half_mask: HalfMask = SOLVER_DATA.completed_half_masks
                    [self.completion.tier as usize]
                    & piece_half_mask;
                let focus_tier_half_mask: HalfMask = SOLVER_DATA.completed_half_masks
                ...
                Grid::new(0).show(ui, |ui: &mut Ui| {
                    for (label, half_mask_string, focus) in [
                        (
                            "Affected Poses",
                            self.full_mask.affected_poses_string(),
                            focus_poses,
                        ),
                        (
                            "Affected Pieces",
                            self.full_mask.affected_pieces_string(),
                            focus_pieces,
                        ),
                    ] {
                        ui.label(label);
                        ui.horizontal(|ui: &mut Ui| {
                            ...
                            for (bit, bit_char) in half_mask_string.chars().enumerate() {
                                ui.label(RichText::new(bit_char).monospace().color(if focus {
        ...
        if let Ok(full_mask) = full_addr.get_full_mask().try_get() {
            (SOLVER_DATA.completed_half_masks[self.completion.tier as usize]
                ...
                & self
                    .completion
                    .solve_stage
                    .get_focus_affected_half_mask(full_mask)
                == ZERO_HALF_MASK
        } else {
            false
        }
...
impl PartialEq for FullMaskAndCompletion {
    fn eq(&self, other: &Self) -> bool {
        self.completion == other.completion
            && self.get_incorrect_piece_count() == other.get_incorrect_piece_count()
    }
}

impl PartialOrd for FullMaskAndCompletion {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        let completion_partial_cmp: Option<Ordering> =
            self.completion.partial_cmp(&other.completion);

        if completion_partial_cmp.is_none() || completion_partial_cmp.as_ref().unwrap().is_ne() {
            completion_partial_cmp
        } else {
            other
                .get_incorrect_piece_count()
                .partial_cmp(&self.get_incorrect_piece_count())
        }
    }
}
...
impl<'p> From<(&'p PuzzleState, StatefulSolveStage)> for ExplorerParams<'p> {
    fn from((puzzle_state, stateful_solve_stage): (&'p PuzzleState, StatefulSolveStage)) -> Self {
        (
            puzzle_state,
            FullMaskAndCompletion::from((
                puzzle_state.arrays(),
                SolveStage::from(stateful_solve_stage),
            )),
...
impl<'p> From<(&'p PuzzleState, FullMaskAndCompletion, StatefulSolveStage)> for ExplorerParams<'p> {
    fn from(
        (puzzle_state, full_mask_and_completion, stateful_solve_stage): (
            &'p PuzzleState,
        ...
        Self {
            ...
            should_explore: Some(
                Box::new(move |_: &PuzzleState, full_addr: FullAddr| -> bool {
...
impl From<StatefulSolveStage> for SolverState {
    fn from(stateful_solve_stage: StatefulSolveStage) -> Self {
        Self::Solving(stateful_solve_stage)
    }
}
...
impl SolverState {
    ...
    fn is_done(&self) -> bool {
        matches!(self, Self::Solved | Self::DidNotFinish)
    }

    #[inline(always)]
    fn is_solving(&self) -> bool {
        matches!(self, Self::Solving(_))
    }
...
impl From<SolveStage> for SolverState {
    fn from(solve_stage: SolveStage) -> SolverState {
        match solve_stage {
            SolveStage::PositionPents => SolverState::Solving(StatefulSolveStage::PositionPents(
                PositionPentsStage::default(),
            )),
            SolveStage::RotatePents => {
                SolverState::Solving(StatefulSolveStage::RotatePents(RotatePentsStage::default()))
            }
            SolveStage::PositionTris => SolverState::Solving(StatefulSolveStage::PositionTris(
                PositionTrisStage::default(),
            )),
...
impl From<u8> for StatType {
    fn from(stat_type: u8) -> Self {
        unsafe { transmute::<u8, Self>(stat_type.min(Self::Count as u8)) }
    }
}
...
impl Solver {
    ...
    #[inline(always)]
    fn is_done(&self) -> bool {
        self.solver_state.is_done()
    }

    #[inline(always)]
    fn is_solved(&self) -> bool {
        self.solver_state.is_solved()
    }
    ...
    fn run_cycle(&mut self, mut max_cycle_duration: Duration) {
        ...
        let mut stateful_solve_stage: StatefulSolveStage =
            solver_state.try_get_stateful_solve_stage().unwrap();
        let mut stat_type_starts: [Instant; StatType::Count as usize] =
            [Instant::now(); StatType::Count as usize];
    ...
    fn build_pending_actions(&mut self, preferences: &Preferences) -> PendingActions {
        let pending_actions: PendingActions = PendingActions {
            ...
            actions: take(&mut self.path)
                .into_iter()
                .map(|transformation: FullAddr| -> Action {
...
mod tests {
    use {
        super::*,
        crate::{
            preferences::{FileMenuData, Preferences, RandomizationParams},
    ...
    fn test_solve_stage_solution(solve_stage: SolveStage) {
        ...
        for iteration in 0_u32..SOLVE_ITERATIONS {
            ...
            while solver.is_solving()
                && solve_stage_from_solver(&solver) == solve_stage
                && solve_cycles != 0_u32
            {
            ...
            match solver.solver_state {
                ...
                SolverState::Solving(stateful_solve_stage) => {
                    if SolveStage::from(stateful_solve_stage) == solve_stage {
                        ...
                        panic!(
                            "Couldn't solve {:?} after 1 minute of solve time on iteration {}\n\
                            SolverState: {:#?}\n\
                            SolverStats: {:#?}",
