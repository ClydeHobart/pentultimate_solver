...
pub trait Addr {
	fn get_genus_index(&self)			-> usize { unimplemented!(); }
	fn get_species_index(&self)			-> usize { unimplemented!(); }
	fn get_species_large_index(&self)	-> usize { unimplemented!(); }
	fn get_organism_index(&self)		-> usize { unimplemented!(); }
...
impl HalfAddr {
	...
	#[inline(always)]
	pub fn as_simple(self) -> FullAddr { FullAddr::from((GenusIndex::SIMPLE, self)) }
	...
	#[inline(always)]
	pub fn is_valid(&self) -> bool { self.0 != Self::INVALID && self.organism_index_is_valid()}

	#[inline(always)]
	pub fn species_index_is_valid(&self) -> bool { Self::is_valid_species_index(unsafe { self.get_species_index_unchecked() }) }

	#[inline(always)]
	pub fn organism_index_is_valid(&self) -> bool { Self::is_valid_organism_index(unsafe { self.get_organism_index_unchecked() }) }

	pub fn get_orientation(self) -> Option<&'static Quat> {
		if self.is_valid() { Some(&Library::get_orientation(self)) } else { None }
	}

	pub const fn default() -> Self { Self(Self::INVALID) }

	#[inline(always)]
	pub const fn is_valid_species_index(species_index: usize) -> bool { species_index < Library::SPECIES_PER_GENUS }

	#[inline(always)]
	pub const fn is_valid_species_large_index(species_large_index: usize) -> bool { species_large_index < Library::SPECIES_PER_LARGE_GENUS }

	#[inline(always)]
	pub const fn is_valid_organism_index(organism_index: usize) -> bool { organism_index < Library::ORGANISMS_PER_SPECIES }

	pub const fn new(species_index: usize, organism_index: usize) -> Self {
		if Self::is_valid_species_large_index(species_index) && Self::is_valid_organism_index(organism_index) {
...
impl Add<FullAddr> for HalfAddr {
	...
	fn add(self, rhs: FullAddr) -> Self {
		if self.is_valid() && rhs.is_valid() {
			let mut sum: Self = rhs.get_transformation().unwrap().as_ref().half_addr(self.get_species_index());

			*sum.set_organism_index((sum.get_organism_index() + self.get_organism_index()) % Library::ORGANISMS_PER_SPECIES)
		} else {
			Self::default()
		}
	}
...
impl From<(usize, usize)> for HalfAddr {
	fn from((species_index, organism_index): (usize, usize)) -> Self { Self::new(species_index, organism_index) }
}
...
impl<'a> From<RandHalfAddrParams<'a>> for HalfAddr {
	fn from(rand_half_addr_params: RandHalfAddrParams<'a>) -> Self {
		...
		*Self::default()
			...
			.set_organism_index((rand_half_addr_params.thread_rng.gen::<f32>() * ORGANISMS_PER_SPECIES_F32) as usize)
	}
}
...
impl Debug for HalfAddr {
	fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
		formatter
			...
			.field("species_index", unsafe { &self.get_species_index_unchecked() })
			.field("organism_index", unsafe { &self.get_organism_index_unchecked() })
			.finish()
	}
}
...
impl Neg for HalfAddr {
	...
	fn neg(self) -> Self::Output { *self.as_reorientation().get_inverse_addr().get_half_addr() }
}
...
impl FullAddr {
	...
	pub fn get_simple_slice_string(self) -> String {
		...
		for (simple_index, simple)
			in self.get_simple_slice().iter().enumerate()
		{
			write!(
				simple_slice_string,
				"{0}{1:\t>2$}ha!({3},\t{4}),",
	...
	pub fn get_inverse_addr(self) -> Self {
		if self.is_valid() { *Library::get_inverse_addr(self) } else { Self::default() }
	}
	...
	#[inline(always)]
	pub fn is_genus_index_reorientation(&self) -> bool { self.genus_index.is_reorientation() }

	#[inline(always)]
	pub fn is_genus_index_simple(&self) -> bool { self.genus_index.is_simple() }

	#[inline(always)]
	pub fn is_valid(&self) -> bool { self.genus_index_is_valid() && self.half_addr_is_valid() }
	...
	#[inline(always)]
	pub fn genus_index_is_valid(&self) -> bool { self.genus_index.is_valid() }
	...
	pub fn get_transformation(self) -> Option<&'static Transformation> {
		if self.is_valid() { Some(Library::get_transformation(self)) } else { None }
	}
	...
	pub const fn invert_organism_index(organism_index: usize) -> usize {
		const ORGANISM_INDEX_LUT: [u8; Library::ORGANISMS_PER_SPECIES] = [0_u8, 4_u8, 3_u8, 2_u8, 1_u8];
...
impl Add<HalfAddr> for FullAddr {
	...
	/// self.half_addr += rhs; self }` because Simple transformations don't have their organism indices affected by
	/// reorientations.
...
impl Debug for FullAddr {
	fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
		...
		debug_struct.field("genus_index", &self.try_get_genus_index().unwrap_or_default());
...
impl From<GenusIndex> for FullAddr {
	fn from(genus_index: GenusIndex) -> Self {
		Self {
			genus_index: if genus_index.is_valid() { genus_index } else { GenusIndex::INVALID },
			.. Self::default()
		}
	}
}
