...
pub const PHI: f64 = 1.618033988749895_f64;
pub const ONE_OVER_PHI: f64 = 0.6180339887498949_f64;
...
pub mod two_d {
    ...
    pub fn compute_interpolant(
        ...
        p3: Vec2,
        p4: Vec2,
    ) -> ComputeInterpolantResult<[f32; 2_usize]> {
        /* This uses the math from Paul Bourke (though not the code from Damian Coventry) from the
        City College of New York: http://www-cs.ccny.cuny.edu/~wolberg/capstone/intersection/Intersection%20point%20of%20two%20lines.html */
        ...
        match optional_cir.unwrap_or_else(|| compute_interpolant(p1, p2, p3, p4)) {
            ComputeInterpolantResult::<[f32; 2_usize]>::Interpolant(interpolants) => {
                ...
                if (bit_slice[Point::P1 as usize] || interpolants[0_usize] >= -f32::EPSILON)
                    && (bit_slice[Point::P2 as usize]
                        || 1.0_f32 - interpolants[0_usize] <= f32::EPSILON)
                    && (bit_slice[Point::P3 as usize] || interpolants[1_usize] >= -f32::EPSILON)
                    && (bit_slice[Point::P4 as usize]
                        || 1.0_f32 - interpolants[1_usize] <= f32::EPSILON)
                {
            ...
            ComputeInterpolantResult::<[f32; 2_usize]>::Coincident => {
                ComputeIntersectionResult::<Vec2>::Coincident
            }
            ComputeInterpolantResult::<[f32; 2_usize]>::Parallel => {
                ComputeIntersectionResult::<Vec2>::Disjoint
            }
        }
    }
...
pub mod three_d {
    ...
    /// Computes the interpolant (if there is one) necessary to compute the intersection between a
    /// line and a plane in 3D space (taken from [Wikipedia](https://en.wikipedia.org/wiki/Line%E2%80%93plane_intersection))
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// A `ComputeInterpolantResult<f32>` containing either the `Interpolants` variant around an
    /// `f32` in the case of a single intersection, the `Coincident` variant in the case of the
    /// line being fully along the plane, or the `Parallel` variant in the case where the line is
    /// parallel to the plane, but never intersecting. To get the intersection from the `f32`
    /// interpolant, compute `l0 + d * l`, where `d` is the interpolant `f32`.
    pub fn compute_line_plane_interpolant(
        l0: Vec3,
        l: Vec3,
        p0: Vec3,
        n: Vec3,
    ) -> ComputeInterpolantResult<f32> {
    ...
    /// Computes the intersection (if there are any) between a line and a plane in 3D space (taken
    ...
    /// * `p0` - A point on the plane
    /// * `n` - A normal vector to the plane that need not be normalized
    /// * `optional_cir` - Optional argument corresponding to the result of calling
    ///   `compute_line_plane_interpolant()` with these same arguments. If this is `None`, the
    ///   interpolant will be computed using all four args. If this is `Some`, only `l0` and `l`
    ///   will be used to compute the intersection. This is supplied in case the interpolant itself
    ///   is of use.
    ///
    /// # Returns
    ///
    /// A `ComputeIntersectionResult<f32>` containing either the `Intersection` variant around a
    /// `Vec3` in the case of a single intersection, the `Coincident` variant in the case of the
    /// line being fully along the plane, or the `Disjoint` variant in the case where the line is
    /// parallel to the plane, but never intersecting.
    pub fn compute_line_plane_intersection(
        ...
        match optional_cir.unwrap_or_else(|| -> ComputeInterpolantResult<f32> {
            ...
            ComputeInterpolantResult::<f32>::Coincident => {
                ComputeIntersectionResult::<Vec3>::Coincident
            }
            ComputeInterpolantResult::<f32>::Parallel => {
                ComputeIntersectionResult::<Vec3>::Disjoint
            }
        }
    }
}
